...
- **The manifest `<link>` must be emitted with `crossorigin="use-credentials"`** (`useCredentials: true` in the `VitePWA` config). Browsers fetch the web app manifest without cookies by default — even same-origin — so behind Cloudflare Access an anonymous manifest fetch gets bounced to the login gate, Chrome considers the site non-installable, and the browser menu only offers "Create shortcut" instead of "Install app".
