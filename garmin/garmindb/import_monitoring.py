...
class GarminSleepData(JsonFileProcessor):
    ...
    def _process_json(self, json_data):
        ...
        if date is None:
            return 0
        # Parse calendarDate directly as a date string to avoid timezone issues
        if isinstance(date, datetime.datetime):
            day = date.date()
        elif isinstance(date, datetime.date):
            day = date
        elif isinstance(date, str):
            # Parse directly as date string (YYYY-MM-DD format)
            day = datetime.datetime.strptime(date.split('T')[0], '%Y-%m-%d').date()
        else:
            # Fallback to original behavior
            day = date.date() if hasattr(date, 'date') else datetime.date.today()
...
class GarminSummaryData(JsonFileProcessor):
    ...
    def _process_json(self, json_data):
        # Parse calendarDate directly as a date string to avoid timezone issues
        # The calendarDate from Garmin is in YYYY-MM-DD format and should be treated as a date, not datetime
        calendar_date = json_data.get('calendarDate')
        if isinstance(calendar_date, datetime.datetime):
            # If it was already parsed as datetime, extract date component
            # But use the date as-is to avoid timezone conversion issues
            day = calendar_date.date()
        elif isinstance(calendar_date, datetime.date):
            day = calendar_date
        elif isinstance(calendar_date, str):
            # Parse directly as date string (YYYY-MM-DD format)
            day = datetime.datetime.strptime(calendar_date.split('T')[0], '%Y-%m-%d').date()
        else:
            # Fallback to original behavior
            day = calendar_date.date() if hasattr(calendar_date, 'date') else datetime.date.today()
